...
async function doStart(): Promise<express.Application> {
  ...
  // start the server
  server = app.listen(props.webPort, function() {
...
async function doStop(): Promise<void> {
  ...
  await new Promise((resolve) => {
    if (!server) {
      warn('HTTP Server is not listening');
      resolve();
      return;
    }
    server.close((err) => {
      if (err) {
        warn('HTTP Serve close error: %s', err);
        resolve();
        return;
      }
      info('HTTP Server is closed');
      resolve();
      return;
    });
  });
...
// Start the application if this module is provided as a command line argument.
