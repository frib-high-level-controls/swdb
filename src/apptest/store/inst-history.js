...
describe("Installations history tests suite", function () {
  ...
  it("Post a new record with correct history", function (done) {
    supertest
      ...
      .set('Cookie', [Cookies])
      .send({host: "Test host", name: "Test name", area: [ "Global" ], status: "Ready for install", statusDate: "date 1000", software: "5947589458a6aa0face9a512"})
      .end(async (err, result) => {
        ...
        debug('Got id ' + id);
        let canonObj =
          { host: "Test host", name: "Test name", area: [ "Global" ], status: "Ready for install", statusDate: new Date("date 1000"), software: mongoose.mongo.ObjectId("5947589458a6aa0face9a512")};
        let response = null;
        try {
